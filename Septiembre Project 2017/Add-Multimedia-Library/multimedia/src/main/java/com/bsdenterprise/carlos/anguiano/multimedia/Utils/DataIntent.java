...
public class DataIntent {
    ...
    public static void resultLoadImageMultimedia(Activity activity, Intent data) {
        Log.i(TAG, "resultLoadImageMultimedia: ");
        if (data != null) {
            if (data.hasExtra(EXTRA_RESULT_SELECTED_ALBUM)) {
                Intent intent = new Intent(activity, MainSingleAlbumActivity.class);
                ...
                activity.startActivityForResult(intent, RESULT_MAIN_SINGLE_ALBUM_MULTIMEDIA);
            }
        }
    }

    public static void resultMainSingleAlbumMultimedia(Activity activity, String user, Intent data) {
        Log.i(TAG, "resultMainSingleAlbumMultimedia: ");
        if (data != null) {
            if (data.hasExtra(EXTRA_RESULT_SELECTED_PICTURE)) {
                Intent i = new Intent(activity, ShowMediaFileActivity.class);
                ...
                activity.startActivityForResult(i, RESULT_IMAGE_SELECTED_MULTIMEDIA);
            }
            if (data.hasExtra(EXTRA_RESULT_SELECTED_VIDEO)) {
                Intent videoPlayer = new Intent(activity, VideoPlayerActivity.class);
                ...
                activity.startActivityForResult(videoPlayer, RESULT_VIDEO_SELECTED_MULTIMEDIA);
            }
        }
    }

    public static void resultVideoSelectedMultimedia(Activity activity, Intent data) {
        ...
        if (data != null) {
            if (data.hasExtra(EXTRA_MEDIA_PATHS_VIDEO)) {
                ...
                if (mImagePaths != null) {
                    ...
                    i.putExtra(EXTRA_TYPE_BUCKET, data.getStringExtra(EXTRA_TYPE_BUCKET));
                    i.putExtra(EXTRA_TYPE_FILE, data.getStringExtra(EXTRA_TYPE_FILE));
                    activity.startActivityForResult(i, RESULT_VIDEO_SELECTED);
                }
            }
        }
    ...
    public static void resultCancelSingleMultimedia(Activity activity) {
...
//        intent.putExtra(EXTRA_NAME, user);
        activity.startActivityForResult(intent, RESULT_LOAD_IMAGE_MULTIMEDIA);
    }
